...
def perform_row_operation(matrix, size, row_operation):
...
def print_history(matrixHistory, operationHistory):
    os.system('cls')
    for i, matrix in enumerate(matrixHistory):
        size = [len(matrix), len(matrix[0])]
        print('-' * 7*3)
        print(operationHistory[i])
        print('-' * 7*3)
...
while True:
    ...
    matrixHistory.append(deepcopy(matrix))
    ...
    while True:
        ...
        if row_operation == 'q':
            print_history(matrixHistory, operationHistory)
            break
        
        if row_operation == 'b':
            if len(matrixHistory) > 1:
                matrix = matrixHistory[-2]
                matrixHistory.pop()
            ...
            matrix = perform_row_operation(matrix, size, row_operation)
            
            # record it in history
            matrixHistory.append(deepcopy(matrix))
            operationHistory.append(row_operation)
